...
docset: $(MANUAL_PACKED_PATH) $(PYTHON_VENV_PATH)
	...
	$(PYTHON_VENV_ACTIVATE) && $(SCRIPT_PATH)/mkindex.py $(MANUAL_UNPACKED_PATH) $(DOCSET_DOCUMENTS_PATH) $(DOCSET_INDEXDB_PATH)

	# Create the Property List file that describes the docset
	$(PYTHON_VENV_ACTIVATE) && $(SCRIPT_PATH)/mkinfo.py $(DOCSET_BASENAME_NO_EXT) "$(DOCSET_READABLE_NAME)" $(DOCSET_SEARCH_KEYWORD) $(DOCSET_MAIN_PAGE) $(OCAML_RELEASE_URL) $(DOCSET_INFO_PATH)
